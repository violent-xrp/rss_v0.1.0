...
- **181 test functions / 3013 assertions / 0 failures** via `python tests/test_all.py`
- **92.7% statement coverage** via `python run_coverage.py`
- **181 claims / 181 tests / 125 Pact sections** in `docs/claim_matrix.md`
- **26 source modules** in the `src/rss/` package tree (R1 restructure complete)
<!-- END GENERATED -->

### Added / hardened
- Subsequent KERNEL-02 candidate: observed authorization expiry is terminal but never a successful claim; claimed state is installed only after the durable claim log returns. This addresses the two broker defects left open by the earlier revalidation entry below. Confirmed commit-then-error and unknown-outcome refusal retain Runtime's TRACE semantics. Whole-lifecycle transactions, concurrency, restart leases and result-storage failure handling remain unclaimed; see `docs/ACTION_PLANE.md`.
...
- Bootstrap now verifies the configured Section 0 Genesis artifact after TRACE preflight and before normal term initialization, migration receipts/schema stamping, state restoration, or default EXECUTE consent. Missing required or mismatched production Genesis persists Safe-Stop and returns only `SafeStopRecovery`; valid production and missing-artifact dev mode retain their documented behavior. Constructor-before-Genesis ordering, constructor-time migration, and full-Pact integrity remain outside this slice.
- Bootstrap now validates every durable row claiming the critical `GLOBAL:EXECUTE` consent before restore or default authority. Unknown status, canonical-key/tuple mismatch, blank requester, duplicate shadow rows, and consent-load failure persist Safe-Stop and return only `SafeStopRecovery`; failed or unfenced refusal closes and raises, invalid rows remain evidence, and neither restore mode calls OATH authorization first. Other consent tuples, schema-level tuple uniqueness, lossless consent-field persistence, atomic Safe-Stop entry, and external-writer races remain outside this bounded slice.
...
- `docs/PROJECT_STATUS.md` is now generated by `docs/build_project_status.py` as a public current-state view with proof snapshot, deterministic drift light, and reviewer doc index.
