...
- v0.1.0 cold verification proves internal chain consistency, schema readability, insertion-order linkage, optional container-filtered views, optional registry coverage, and v2 stored-field recomputation for new TRACE rows. v1 historical rows remain linkage-only because their raw payloads were never persisted. v2 proves persisted-field integrity inside the SQLite record; it still does not prove payload-inclusive external recomputability because raw canonical payloads are not exported for third-party recomputation.
...
- New TRACE rows use `hash_version=2`: `content_hash` recomputes from timestamp, event code, authority, artifact ID, payload hash, byte length, and parent hash. Cold verification fails on missing v2 payload hashes, in-place field edits, and hash-version downgrade attempts; JSON export includes `payload_hash` and `hash_version` for visibility.
...
- `production_mode` is a real single switch in `RSSConfig.__post_init__`: it forces strict event-code validation, lowers audit failure threshold to 1, disables console logging, requires the Genesis file, and raises SQLite synchronous mode to `FULL`.
...
- SQLite persistence accepts only whitelisted synchronous modes. Development defaults to `NORMAL`; `production_mode=True` forces `FULL`. `OFF` is rejected rather than becoming a hidden durability downgrade.
...
- REDLINE export sanitization is token-boundary based: known REDLINE entry IDs are replaced in artifact identifiers without over-redacting unrelated larger tokens. Cold export collects REDLINE IDs through `Persistence.redline_entry_ids()` across both global and container hub tables, with legacy SQLite fallback retained for older persistence objects.
...
- Cold verification and cold export now prove chain consistency, export hygiene, and v2 persisted-field recomputation for new rows, not full payload-inclusive external recomputability. Any stronger claim requires exported canonical proof material that allows an independent verifier to recompute event hashes from payload material, plus REDLINE/privacy policy for what payload material may leave the runtime. Phase H needs signed/timestamped payload receipts or export bundles before that stronger claim is made.
- The minimum future external recomputation bundle would need the exact hash-envelope inputs: chain hash version, timestamp string, event code, authority, unsanitized artifact ID, normalized raw content payload or payload sidecar, parent hash, and canonical JSON serialization rules. The current `trace_events` table stores `payload_hash`, not raw content payload, so this is not a payload-authenticity or third-party recomputation proof claim.
...
- Thread safety is currently single-process with WAL plus process-local locks around persistence and audit-chain append/persist order. Multi-process and distributed persistence remain future posture, not current proof.
...
- Claim-time revalidation shares those current checks except CYCLE, which is charged only at review. New governance refusals remain unclaimed; a later retry must pass again within the existing proposal and authorization TTLs. This is sequential checking, not an atomic snapshot through external execution.
...
- Execution result import enters through `save_untrusted_content()` as data-only evidence; result text does not become authority. The KERNEL-02 candidate separates observed expiry from successful claim and publishes claimed state only after the durable claim log returns. This is sequential write-ahead ordering, not a transaction across lease issuance/revocation, result storage, crashes or concurrent calls; `ACTION_PLANE.md` owns the remaining lifecycle limits.
