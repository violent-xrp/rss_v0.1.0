...
- Execution result import checks an internal claimed flag and enters through `save_untrusted_content()` as data-only evidence; result text does not become authority. The pre-existing authorization-expiry flag and successful-claim/receipt ordering defects prevent treating that flag as a strict successful-claim guarantee; `ACTION_PLANE.md` owns these known limits.
