...
- generated project status: `docs/PROJECT_STATUS.md`
...
Current code state:
- **178 test functions / 1820 assertions / 0 failures** via the custom acceptance runner (`python tests/test_all.py`)
- **92.4% statement coverage** via `python run_coverage.py`
- **178 claims / 178 tests / 120 Pact sections** in `docs/claim_matrix.md`
...
- public docs are synced to the current 178/1820 baseline
...
- **Production Genesis before authority:** closed as a bounded bootstrap slice. After TRACE continuity is restored and any pre-existing halt is routed to recovery, bootstrap verifies the configured Section 0 artifact before normal term initialization, migration receipts/schema stamping, governed-state restore, or default EXECUTE consent. Missing required or mismatched Genesis establishes persistent Safe-Stop and returns `SafeStopRecovery`; valid production and documented missing-artifact dev mode continue normally. This does not move verification ahead of `Runtime` construction or constructor-time schema migration, extend integrity beyond Section 0, or validate persisted consent rows; those remain separate invariants.
