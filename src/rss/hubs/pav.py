...
from rss.governance.seats.scope import ScopeEnvelope
from rss.hubs.topology import HubTopology, HubEntry
...
class PAVBuilder:
    ...
    def _sanitize(self, entry: HubEntry, policy: str) -> dict:
        ...
        raise ValueError(
            f"Unknown PAV policy: {policy!r}. "
            f"Valid policies: {CONTENT_ONLY!r}, {CONTENT_HUB!r}, "
            f"{CONTENT_HUB_TIME!r}, {FULL_CONTEXT!r}"
        )
